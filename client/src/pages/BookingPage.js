    ...
    try{
      ...
      console.log(error)
        ...
        const handleAvailability=async()=>{
          try{
            dispatch(showLoading())
            const res = await axios.post('/api/v1/users/booking-availbility',{
              doctorId:params.doctorId,
              date,time},{
                headers:{
                  Authorization: `Bearer ${localStorage.getItem('token')}`
      ...
      <div className="container m-2">
        {doctors && (
            ...
            <div className='d-flex flex-column w-50'>
                <DatePicker aria-required={'true'} className='m-2' format='DD-MM-YYYY' onChange={(value)=>{setDate(moment(value).format('DD-MM-YYYY'))}} />

                <TimePicker aria-required={'true'} className='m-2' format='HH:mm' onChange={(value)=>{setTime(moment(value).format('HH:mm'))}} />
